    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(strUsername != null) {
            try {
                ...
                request.getRequestDispatcher("/pages/admin/user-update.jsp").forward(request, response);
                
            }
            catch(Throwable t) {
                logger.warn("Unable to load user with user_id '" + strUsername + "'");
                request.getRequestDispatcher("/pages/admin/user-search.jsp").forward(request, response);
        ...
        if(Action.equalsIgnoreCase("Update")){
            ...
            try {   
                ...
                if(fullname1.compareTo(fullname)!=0)
                    buff.append("old FullName="+fullname1+" new Fullname="+fullname);
                if(nik1.compareTo(nik)!=0)
                    buff.append(",old NIK="+nik1+" new NIK="+nik);
                if(email1.compareTo(emailaddress)!=0)
                    buff.append(",old Email="+email1+" new Email="+emailaddress);
                if(department1.compareTo(department)!=0)
                    buff.append(",old department="+department1+" new Department="+department);
                if(division1.compareTo(division)!=0)
                    buff.append(",old division="+division1+" new Division="+division);
                if(status1!=status)
                    buff.append("old status="+status1+" new Status="+status);
                request.setAttribute(HttpConstants.ATTR_NAME_USER, user);    
            ...
            try {
                ...
                pstmt.setString(3, "Update "+ user.getUsername()+" by "+userSession.getUser().getUsername()+" Changed was done :" +buff.toString());
                pstmt.executeUpdate();
         ...
         //  request.getRequestDispatcher("/pages/admin/user-update-success.jsp").forward(request, response);
            ...
            request.getRequestDispatcher("/pages/admin/user-delete-success.jsp?message=reset&nik="+user.getNik()).forward(request, response);
        }
    }
   }
}
